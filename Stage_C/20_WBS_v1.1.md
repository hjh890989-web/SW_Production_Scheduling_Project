...
| 문서 번호 | 20 |
| 버전 | **v1.1** (2026-06-01 개정) |
| 작성일 | 2026-05-10 (v1.0), 2026-06-01 (v1.1) |
| Owner 팀 | 경영기획 본부 |
| 문서 성격 | **GitHub Issue 추출용 Task 마스터 리스트** — 13 Sprint × 평균 6 Task = **117 Task** |
| 입력 자료 | PRD #19 v1.4 (특히 §3 70 AC, 부록 I Sprint 분할), ADR D-22 (TBD-5 종결), Stage_B/15 JTBD 인터뷰 결과 |
| 다음 산출물 | #21 Issue 추출 프롬프트 + Stage_D/issues/ 개별 명세서 (이미 v1.0 기반 115건 발행, v1.1로 +2건 누적) |
| v1.1 주요 변경 | ① **T0.8 신규** (간트 라이브러리 PoC — TBD-1 Sprint 5 진입 전 결정) ② **T5.12 신규** (베타 사용성 테스트 — ADR D-22, TBD-5b) ③ T5.1 범위 축소 (결정→통합) |
...
[3] Stage_D/issues/T{ID}_{slug}.md 형태로 저장
...
# 2. Sprint 0 — 프로젝트 셋업 (Must, 약 1주, 8 Task)
...
| **T0.8 🆕** | **간트 라이브러리 PoC (TBD-1 결정)** | Bryntum / DHTMLX / frappe-gantt 3종 PoC + 비교표 + `docs/decisions/D-23_gantt_library.md` 작성. Sprint 5 T5.1 진입 전 결정 완료 | TBD-1 | T0.3 | 4 |

**Sprint 0 합계: 약 31h ≈ 4일** (v1.0 27h → v1.1 31h, +T0.8 4h)
...
# 7. ⭐⭐ Sprint 5 — 성형 스케줄러 (Must, 약 3주, 12 Task) ★ J-MR-2 도입 성패

| Task ID | 작업명 | 핵심 내용 | AC/F/R | 의존성 | 예상 (h) |
|---|---|---|---|---|---|
| **T5.1** | 간트 라이브러리 **통합** | T0.8에서 결정된 라이브러리 설치 + 도메인 데이터 어댑터 + 기본 렌더링 | - | T0.3, T0.8 | 6 |
...
| **T5.12 🆕** | **베타 사용성 테스트 (TBD-5b 종결)** | 박철수·이영호 반장 hands-on 1일. 만족도 ≥4/5 검증. AC T5.12-F1 미달 시 Sprint 5 중단 → ADR D-24 (J-MR-2 재설계) | KSF-6, ADR D-22 | T5.4, T5.6 (부분) | 8 |

**Sprint 5 합계: 약 86h ≈ 11일** ⭐⭐ 가장 중요한 Sprint (v1.0 78h → v1.1 86h, +T5.12 8h)
...
| **합계** | **117 Task** (+2) | **약 662h** (+12) | **약 87일 (4개월 1인)** | |

> ⚠️ **시간 추정 주의**: 단순 코딩 시간만. 검토·디버그·재작업·문서화 등 포함 시 1.5~2배 가능.

## 21.2 Must (P0 — MVP) 합계
- Sprint 0~8 + Sprint 11 = **76 Task** (+2), **약 451h** (+12), **약 57일**
- → **MVP 출시까지 약 8주 (1인)**, 2인 페어 시 약 5~6주 (소폭 증가, 일정 변동 미미)
...
> **v1.1 영향**: T0.8(간트 PoC)·T5.12(사용성 테스트) 모두 Critical Path **외부**(병렬 가능). Critical Path 자체는 변동 없음.

병렬 가능 작업 묶음:
- T0.2·T0.3 (Tailwind·shadcn)
- T0.5·T0.6 (Auth·Docker)
- **T0.8 (간트 PoC) — T0.3 완료 후 병렬** 🆕
- T2.2·T2.3·T2.4 (시드 데이터)
- T5.6·T5.7·T5.8 (성형 UI 인터랙션)
- **T5.12 (사용성 테스트) — T5.4 완료 직후 T5.6과 병렬 가능** 🆕
...
| 그룹 | Task 범위 | 작업 분량 |
|---|---|---|
| G1 | T0.1~T0.8 (Sprint 0 전체, **8**) | 1 회 |
...
| G7 | T5.6~T5.12 (Sprint 5 ⭐⭐ 핵심 + 사용성 테스트, **7**) | 1 회 |
...
→ **총 약 14~15회 Issue 추출 작업** 필요. (v1.1 영향: G1/G7 각 +1건. Stage_D/issues는 v1.0 기준 115건 발행 완료 — v1.1 추가분 [T0.8](../Stage_D/issues/), [T5.12](../Stage_D/issues/T5.12_beta-usability-test.md) 중 T5.12는 발행, **T0.8은 후속 발행 필요**.)
...
| 단계 | 산출물 | 시점 |
|---|---|---|
| **1** | **T0.8 Stage_D 이슈 명세서 발행** (Stage_D/issues/T0.8_gantt-library-poc.md) | 다음 (30분) |
| **2** | T0.1 Next.js 프로젝트 생성 (Sprint 0 시작) | 그 후 (1시간) |
| 3 | G1 (Sprint 0, 8 Task) 순차 진행 → 첫 PR | 그 후 (1주) |
| 4~ | Sprint별 Task 진행 → 코드 → 검증 반복 | 8주 (MVP 기준) |
...
| **v1.1** | **2026-06-01** | **누락 Task 2건 추가**: ① **T0.8 간트 라이브러리 PoC** (TBD-1 Sprint 5 진입 전 결정 부담 해소, Sprint 0에 4h) ② **T5.12 베타 사용성 테스트** (ADR D-22 TBD-5b 종결, Sprint 5에 8h, 박철수·이영호 반장 hands-on). T5.1 범위 축소(결정→통합). 총 **117 Task / 662h**. Critical Path 영향 없음 (병렬 task) | PM + Claude |
