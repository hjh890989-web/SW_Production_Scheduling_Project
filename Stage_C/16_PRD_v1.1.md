# 사내 생산 스케줄링 시스템 — PRD v1.1

| 항목 | 내용 |
|---|---|
| 문서명 | PRD — 통합 요구사항 명세서 |
| 문서 번호 | 16 |
| 버전 | **v1.1** (전면 보강 — 9개 섹션 + 5개 규칙 적용) |
| 최종 업데이트 | 2026-05-10 |
| Owner 팀 | 경영기획 본부 |
| 문서 성격 | **AI 페어 코딩 직접 구현용 PRD** (외주용 RFP 아님) |
| 입력 자료 | 개발계획서 #4 / 문제정의서 #6 / 페르소나 #8 / 여정맵 #9 / JTBD #10 / VPS #13 / PRD v1.0 #15 |
| 다음 산출물 | #17 와이어프레임 (선택) 또는 Sprint 0 코드 시작 |
...
| 목표 (Target) | **≥ 99%** (12개월 후) |
| 측정 주기 | **일 단위** 자동 산출, **주간** 집계, **월간** 보고 |
| 데이터 출처 | DB 스케줄 + MES 실적 |

### 보조 KPI (5개)

| KPI | 기준선 | 목표 | 측정 주기 | 출처 |
|---|---|---|---|---|
| KSF-2 다이/노즐+금형 변경 | 일 5~10회 | -30% | 일 단위 | MES 실적 |
| KSF-3 변동 영향 시간 | 4시간 | ≤ 5분 | 변동 발생 시 | App 로그 |
| KSF-4 스케줄링 작업시간 | 주 24h | -50% | 주 단위 | 사용자 자가보고 + App 사용 시간 |
| KSF-5 데이터 일원화 | 0% | 100% | 월 단위 | DB 적재율 |
| KSF-6 시스템 채택률 | - | ≥ 90% (3개월 후) | 월 단위 | 활성 사용자 / 대상 사용자 |
...
# 3. ★ 사용자 스토리와 수용 기준 (AC, Given/When/Then × 3)

본 PRD의 **검증 가능한 코딩 사양**. 14개 JTBD 모두를 사용자 스토리로 변환, 각 스토리당 AC 3개 + 측정 가능한 임계치.
...
| 차원 | 현재 운영 (대안) | 본 시스템 | 배수 |
|---|---|---|---|
| **속도 (변동 영향 파악)** | 4시간 | 5분 | **48배 ↑** |
| **속도 (슬롯 결정)** | 0.5~1시간 | 5초 + 검토 5분 | **6~12배 ↑** |
| **정확도 (위치 제약)** | 사람 한계 (한 번에 계산 불가) | 100% 자동 반영 | **인지 한계 돌파** |
| **데이터 일원화** | 0% (엑셀 분산) | 100% (단일 DB) | **신규 자산** |
| **인건비 (스케줄링 시간)** | 주 24h | 주 12h | **연 ≈ 1,000h 절감** |
| **납기 사고율** | 7% | ≤ 1% | **-86%** |
| **다이/노즐 변경 횟수** | 일 5~10회 | -30% | **-30%** |
| **분기 보고서 작성** | 며칠 | 30분 | **수십 배 ↑** |
| **베테랑 의존** | 1인 (위임 불가) | 시스템 + 후임 N명 가능 | **N배 ↑** |

→ **속도·정확도·비용·확장성 모두 의미 있는 차이**, 핵심 차원 4가지 모두 10배 이상.
...
| 영역 | 임계치 (p95) | 측정 지점 |
|---|---|---|
| 통합 대시보드 첫 로드 | **≤ 1500ms** | 사내망 LAN 기준 |
| 페이지 전환 | **≤ 500ms** | 클라이언트 라우팅 |
| 변동 영향 시뮬 | **≤ 5000ms** | Server Action 응답 |
| 자동 스케줄 생성 (47품번) | **≤ 5000ms** | Server Action |
| 슬롯 드래그 반영 | **≤ 500ms** | UI 응답 |
| 엑셀 업로드 (1MB 기준) | **≤ 30000ms** | 매핑·검증 포함 |
| Audit 로그 기록 | **≤ 500ms** | DB 쓰기 |
| 알림 발송 | **≤ 1000ms** | 사용자 수신까지 |
...
### 5.5.1 로그 (필수)
- App 로그 (stdout) — 12-Factor 준수
- Audit 로그 (DB) — 5년 보존
- Error 로그 (Sentry 자체호스팅 또는 사내 ELK 검토)

### 5.5.2 대시보드 (사내 — Grafana 권장)

| 지표 | 알림 임계치 |
|---|---|
| HTTP 5xx 비율 | > 1% / 5분 → 알림 |
| 응답 시간 p95 | > 임계치(§5.1) × 2 / 5분 → 알림 |
| DB 연결 수 | > 80% 풀 → 알림 |
| Disk 사용률 | > 80% → 알림 |
| 백업 실패 | 즉시 알림 |
| 로그인 5회 연속 실패 (특정 사용자) | 즉시 알림 |

### 5.5.3 비즈니스 KPI 모니터링

| KPI | 산출 방법 | 알림 |
|---|---|---|
| 납기 준수율 (KSF-1) | (충족 ÷ 전체) × 100 | < 95% / 일 → 알림 |
| 다이/노즐 변경 (KSF-2) | MES 실적 카운트 | > 일 12회 → 알림 |
| 시스템 채택률 | 활성 / 등록 사용자 | < 80% / 월 → 알림 |
...
| **v1.1 (#16)** | **2026-05-10** | **9개 섹션 전면 보강**: ① Pain·Outcome 수치화 / ② 14건 User Story + AC(Given/When/Then × 3) / ③ 북극성 KPI + 보조 KPI + 측정 주기 / ④ Differential Value 수치 비교 / ⑤ Proof 통합 / ⑥ MoSCoW (Must/Should/Could/Won't) / ⑦ 실험·롤아웃·측정 신설 / ⑧ Mermaid 다이어그램 (ERD·여정·롤아웃·데이터흐름) / ⑨ 기존 구현 상세 부록 정리. 가정·의존성·NFR 정량화 (p95·가용성·모니터링) | 경영기획 본부 |
