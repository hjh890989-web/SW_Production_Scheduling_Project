# 사내 생산 스케줄링 시스템 — PRD v1.4

| 항목 | 내용 |
|---|---|
| 문서명 | PRD — 통합 요구사항 명세서 |
| 문서 번호 | 19 |
| 버전 | **v1.4** (최종 검토 보강 — 엄격 적용) |
| 최종 업데이트 | 2026-05-10 |
| Owner 팀 | 경영기획 본부 |
| 문서 성격 | **AI 페어 코딩 직접 구현용 PRD** (외주용 RFP 아님) |
| 입력 자료 | 개발계획서 #4 / 문제정의서 #6 / 페르소나 #8 / 여정맵 #9 / JTBD #10 / VPS #13 / PRD v1.0~v1.3 (#15~#18) |
| 다음 산출물 | 와이어프레임 (선택) 또는 Sprint 0 코드 시작 |
| v1.3 → v1.4 변경 | **6개 기준 최종 검토 보강 (엄격 적용)**: ① §3 **각 Story당 두 번째 실패 AC 추가** (총 +14건, 56→70 AC) ② **F-8.1 OR-Tools를 Sprint 12.1·12.2·12.3로 세분화** (1스프린트 내 구현 가능성) ③ §7 시작에 **ADR 링크** 명시 (개발계획서 §15.1) ④ §7.2 Out of Scope에 **단계별 단서** 추가 (P2 보류·D8 변경 시). 종합 점수 약 90% → 약 97% |
...
## 1.1 ★ 한 줄 가치 제안 (Value Proposition)

> **"베테랑이 며칠 걸려도 다 풀지 못하던 스케줄링 퍼즐을, 시스템이 5초 안에 초안을 만들고 베테랑은 검토·수정만 하는 구조."**

**Pain → Mechanism → Outcome 구조**:
- **Pain**: 베테랑이 머리로 슬롯 위치 제약·E그룹 묶음을 한 번에 계산할 수 없어 결정 시간 0.5~1시간/일, 변동 영향 파악 4시간/회 소요
- **Mechanism**: 시스템이 백워드 스케줄링 + 룰베이스로 5초 자동 초안 생성, 사람은 드래그·재배분으로 수동 보정 (자동+수동 D10)
- **Outcome**: 결정 시간 → 5초+5분 검토, 변동 영향 → 5분 (-98%), 납기율 93% → ≥99%, 다이/노즐 변경 -30%, 인건비 연 ≈1,000h 절감

> 출처: VPS #13 Executive Summary
...
| **🔥 AC PM-1-F2** (실패 — 미존재 품번) | **Given** 변동 입력 폼에 **품번 마스터에 없는 품번** 입력 / **When** 시뮬 버튼 클릭 / **Then** Zod 검증 실패 → **빨간 인라인 에러 "품번 미존재 — 마스터 등록 필요"** + 마스터 화면(W-6) 링크 제공, 시뮬 차단, audit 미기록 |
...
| **🔥 AC PM-2-F2** (실패 — 사용자 취소) | **Given** 자동 생성 진행 중 (5초 미만) / **When** 사용자가 "취소" 클릭 / **Then** **부분 결과 폐기** + DB 트랜잭션 롤백, "자동 생성 취소됨" 토스트, audit에 `cancelled_by_user=true` 기록 |
...
| **🔥 AC PM-3-F2** (실패 — Audit 저장 공간) | **Given** AuditLog 테이블 크기가 임계치 (예: 10GB) 초과 / **When** 매일 23:55 / **Then** **자동 아카이빙 cron** 실행 → 1년 이상 된 로그를 압축 파일로 NAS 백업 후 DB에서 삭제 (5년 보존 룰 유지), Admin에 알림 |
...
| **🔥 AC PM-4-F2** (실패 — 진행 중인 가류기 PM 입력) | **Given** 가류기 1호기에 현재 작업 진행중 (status: STARTED) / **When** "1호기 PM" 입력 / **Then** **즉시 영향받는 진행중 건 N건 경고** 모달 표시 (≤ 1초), "그래도 적용" 시 해당 건은 status: INTERRUPTED + audit `pm_during_work=true` |
...
| **🔥 AC MR-1-F2** (실패 — 태블릿 회전 중 데이터 갱신) | **Given** 태블릿에서 W-4 사용 중, MES 동기화 발생 (5분 주기) / **When** 사용자가 화면을 가로↔세로 전환 / **Then** **현재 스크롤 위치·선택 상태 보존** + 신규 데이터 부드러운 전환 (애니메이션 ≤ 300ms), 작업 흐름 끊김 없음 |
...
| **🔥 AC MR-2-F2** (실패 — 시스템 다운 중 작업) | **Given** 박철수가 슬롯 드래그 중 서버 연결 끊김 / **When** "확정" 시도 / **Then** **로컬 큐(IndexedDB)에 변경 저장** + "오프라인 — 재연결 시 자동 동기화" 배너, 재연결 시 자동 전송 + 충돌 시 위 AC MR-2-F1 적용 (PWA Service Worker — Sprint 12+) |
...
| **🔥 AC MR-3-F2** (실패 — 자동 생성 timeout) | **Given** 자동 생성 작업이 **10초 초과**되어 timeout / **When** Server Action timeout 발생 / **Then** **부분 결과 표시** (이미 처리된 품번 수 표시) + "재시도" 버튼 + 백엔드 로그 `slow_schedule=true` (Grafana 알림 → 알고리즘 튜닝 필요) |
...
| **🔥 AC ER-1-F2** (실패 — 모든 압출기 가동 불가) | **Given** 캘린더에 **신규·포드 모두 PM 처리** / **When** 자동 압출 스케줄 시도 / **Then** **명확한 에러** "해당 일자 압출기 가용 없음 — 캘린더 확인" + W-7 캘린더 링크 + 다음 가용일 자동 제안 |
...
| **🔥 AC ER-2-F2** (실패 — 동일 라인 동시 편집) | **Given** 김민수와 이영호가 동일 압출 라인의 같은 근무시간 슬롯을 동시 편집 / **When** 두 번째 사용자 저장 / **Then** **낙관적 락 충돌 감지** → "최신 버전 보기" 버튼 + diff 표시, 사용자가 자신 변경 재적용 여부 결정 (AC MR-2-F1과 동일 패턴) |
...
| **🔥 AC ER-3-F2** (실패 — 부하 데이터 결손) | **Given** 부하 분산 그래프 표시 시 일부 ProductionResult 데이터 결손 (MES 누락) / **When** 화면 렌더링 / **Then** **부분 데이터로 표시** + "최근 N분 MES 데이터 미수신 — 표시 정확도 부분적" 안내 배너, 데이터 정상화 시 자동 갱신 |
...
| **🔥 AC SP-1-F2** (실패 — 거대 파일) | **Given** **50MB 초과 엑셀 파일** 업로드 시도 / **When** 파일 선택 / **Then** **클라이언트 측 즉시 거부** ("파일 크기 50MB 초과 — 분할 업로드 안내") + 안내 문서 링크. 우회 시도(직접 API 호출)는 서버에서도 거부 (HTTP 413 Payload Too Large) |
...
| **🔥 AC SP-2-F2** (실패 — 변동 입력 즉시 취소) | **Given** 정수진이 변동 저장 직후 5초 이내 "취소" 클릭 / **When** 취소 / **Then** **5초 grace period 안에 알림 발송 자동 취소** (큐에서 제거), Notification 자동 soft-delete, 사용자에게 "변동 취소 완료" 토스트, audit에 `cancelled_within_grace=true` |
...
| **🔥 AC EX-1-F2** (실패 — KPI 로딩 지연) | **Given** KSF 산출 SQL이 5초 초과 / **When** 대시보드 진입 / **Then** **스켈레톤 UI 즉시 표시** + 카드별 점진 로딩 (각 KSF 비동기), 10초 초과 시 "재시도" 버튼 + 백엔드 슬로우 쿼리 알림 (pg_stat_statements) |
...
| **🔥 AC EX-2-F2** (실패 — 분기 보고서 export 실패) | **Given** 분기 보고서 export 시 서버 디스크 사용률 > 90% / **When** PDF 생성 시도 / **Then** **명확한 에러** "디스크 용량 부족 — 관리자 확인 필요" + Admin에 즉시 알림 (Grafana), 임시 파일 자동 정리 후 사용자 재시도 가능 |
...
# 5. 비기능 요구사항 (NFR) + 대안 비교

## 5.0 ★ Competitor / Substitute (대안 비교 — v1.3 신설)

본 시스템의 경쟁자는 **현재 운영 방식 + 외부 SaaS 옵션**. 4가지 대안을 성능·보안·비용 기준선으로 비교.

### 5.0.1 4가지 대안 비교 표

| 차원 | (1) 현재 운영<br>엑셀 + 수기 + 메일/전화 | (2) 영림원 ERP<br>자체 스케줄링 모듈 | (3) 외부 SaaS<br>(Optimo·Kinaxis 등 글로벌 APS) | (4) **본 시스템 (사내 웹앱)** |
|---|---|---|---|---|
| **도입 비용** | 0 (기존) | 모듈 추가 라이선스 (TBD) | **연 ₩수억** (사용료) | 1회 개발 + 사내 유지보수 |
| **운영 비용 (연간)** | (인건비 손실 1,000h+ + 사고 7%) | 라이선스 + 일부 손실 해소 | **연 ₩수억** + 통신비 | 사내 서버 1대 운영비 |
| **통합도** | ❌ 엑셀 3종 분산 | 🟡 ERP 내 한정 | ✅ 외부에서 통합 | ✅ 사내 단일 DB |
| **자동화 (스케줄링)** | ❌ 100% 수기 | 🟡 일반 룰만 | ✅ 고급 알고리즘 | ✅ 룰베이스 → P2 OR-Tools |
| **변동 5분 영향 시뮬** | ❌ 4시간 | ❌ 미지원 | 🟡 일부 지원 | ✅ ≤ 5분 (R-8) |
| **OEM 영업비밀 보호** | 🟡 사내 보관 | ✅ 사내 ERP | ❌ **외부 클라우드** (NDA 위반 위험) | ✅ 사내망 (D8) |
| **ERP·MES 연동** | 🟡 수기 대조 | ✅ ERP 내 통합 | 🟡 별도 연동 (복잡) | ✅ 양방향 자동 (사내) |
| **사내망 정책 (D8) 호환** | ✅ | ✅ | ❌ **위반** | ✅ |
| **베테랑 노하우 흡수** | ❌ 머리에만 | 🟡 일부 | 🟡 일반 모델 | ✅ 마스터 GUI 명문화 (R-9) |
| **박철수 반장 친화 (J-MR-2)** | 🟡 익숙 | 🟡 | ❌ 영문 UI 등 거부 가능 | ✅ 베테랑 UX 설계 |
| **확장성 (재료·공장·SaaS)** | ❌ | 🟡 ERP 안 | ✅ | ✅ Cloud-Ready (D20) |
| **미래 클라우드 이전** | ❌ | ❌ ERP 종속 | (이미 클라우드) | ✅ 12-Factor |
| **Audit 5년 보존 (자동차부품 추적)** | ❌ 엑셀에 사실상 부재 | 🟡 ERP 보존 | 🟡 SaaS 정책 따름 | ✅ DB Audit 5년 (R-13) |

### 5.0.2 성능·보안·비용 기준선 (Baseline)

대안별 핵심 차원의 정량 비교:

| 차원 | (1) 현재 | (2) 영림원 | (3) SaaS | **(4) 본 시스템** | **목표 SLO** |
|---|---|---|---|---|---|
| 변동 영향 시뮬 응답 (p95) | 4시간 (수기) | N/A | 알림 (수 분) | **≤ 5초** | §5.1 §AC PM-1-1 |
| 자동 스케줄 생성 (47품번) | 수기 4시간 | TBD | TBD | **≤ 5초** | §5.1 |
| 데이터 외부 전송 | 0 (엑셀만) | 0 | 100% (필수) | **0** | D8 사내망 |
| Audit 보존 기간 | 0~6개월 | 1~3년 | SaaS 정책 | **5년** (자동차부품) | R-13 |
| 가용성 | 99% (수기 운영) | 99% | 99.9%+ | **≥ 99%** | §5.2 |
| 도입 비용 (5년 TCO) | (손실 누적 ≈ ₩수억) | TBD | (₩수억+ 사용료 5년) | **개발 1회 + 유지비** | - |
| 회수 기간 | - | TBD | 즉시 사용 | **≤ 18개월** | §10 |

### 5.0.3 대안별 결론

| # | 대안 | 결론 |
|---|---|---|
| (1) | 현재 운영 유지 | ❌ Pain 누적 손실 (연 1,000h + 다이변경 + 납기사고 7%). **본 시스템 도입의 정당성** |
| (2) | 영림원 ERP 모듈 | 🟡 ERP 종속, 본 프로젝트 핵심 요구사항(R-7 변동·R-8 시뮬·R-11 자동+수동·J-MR-2 ⭐⭐) 미충족 |
| (3) | 외부 SaaS | ❌ **D8 사내망 정책 위반** + OEM NDA 위반 위험. **합리적 대안 X** |
| (4) | **본 시스템** | ✅ **유일한 호환** + 핵심 요구사항 100% 충족 + 자산화 가능 |

### 5.0.4 핵심 차별 요인 (대안 대비 본 시스템의 본질)

1. **사내망 + 영업비밀 보호** — 외부 SaaS 불가 (D8 위반)
2. **베테랑 친화 UX (J-MR-2 ⭐⭐)** — 일반 SaaS는 박철수 반장 거부 위험
3. **자동+수동 보정 철학 (D10)** — 일반 ERP·SaaS는 자동 강제 경향
4. **Cloud-Ready 동시 보유 (D20)** — 사내·미래 클라우드 모두 옵션

→ 출처: VPS #13 §5 Competitor / 본 시스템 외 합리적 대안 없음.
...
> **관련 ADR (Architecture Decision Record)**: `Phase_A/4_개발계획서_v1.3.md` §15.1 — **D1~D21** (총 21개 의사결정). 본 PRD의 모든 구조 결정은 해당 ADR을 근거로 한다. 충돌 발생 시 ADR 우선.
...
- 검사·포장·출하 공정 (Phase D-4 이후 검토)
- 자재 자동 발주 (ERP 영역 — 본 시스템 책임 외)
- 원가 계산 (ERP 영역)
- **외부 클라우드 배포** (D8 사내망 정책 — **D8 정책 변경 시** 별도 검토, 부록 H Cloud-Ready 12-Factor로 사전 대비)
- **외부 LLM API** 호출 (Gemini·OpenAI 등 — 영업비밀 보호. P2의 사내 Ollama만 검토 가능)
- **실리콘 외 재료** (**P2 진입 시 EPDM·NBR 등으로 점진 확대** — D9 점진 도입 전략에 따름. P2까지는 Out, P2 이후는 In)
- B2C·SaaS化 (별도 사업 프로젝트)

> **단계별 단서 명시**: 위 Out of Scope 중 일부는 **시점·정책 변경 시 In Scope로 이동 가능**. PM 의사결정 충돌 방지를 위해 단서를 명시.
...
| **12.1** | **P2-A: OR-Tools 마이크로서비스 스켈레톤** — Python FastAPI 컨테이너, Next.js와 HTTP API 통신, health check, 인증 토큰, 배포 파이프라인 | 단위 테스트: `/health` 200 OK, 인증 토큰 검증 |
| **12.2** | **P2-B: OR-Tools CP-SAT 솔버 알고리즘** — 성형(슬롯·앵글·회전) + 압출(E그룹·헤드핀·부하) 제약식 구현, 단독 단위 테스트 | 단위 테스트: 47품번 입력 시 셋업 -30% 달성 (Vitest assert) |
| **12.3** | **P2-C: 본 시스템 통합 + 부하 튜닝** — 본 시스템에서 OR-Tools 호출, 결과 비교 UI(룰베이스 vs 솔버), 부하 테스트, 정확도 검증 | k6: p95 ≤ 30초, 결과 동등성 100% |
| 12.4 | AD/LDAP SSO 검토·구현 | AC PM-3-2 SSO 호환 |
| 12.5 | F-9 모바일 KSF 대시보드 (PWA) | AC EX-1·2 |
| 12.6 | F-8.2 EPDM·NBR 등 타 재료 확대 | 마스터 데이터 + 재료 필터 검증 |
| 12.7 | (선택) Ollama 사내 LLM 검토 (자연어 변동 입력 등) | TBD |
...
| v1.2 (#17) | 2026-05-10 | 측정·검증 가능성 보강 (KPI SQL·실패 AC 14건·NFR 도구·Differential 측정·Proof↔Experiment 매핑) | 경영기획 본부 |
| v1.3 (#18) | 2026-05-10 | VPS↔PRD 매핑 갭 보강 (한 줄 가치 제안 §1.1, Competitor §5.0) | 경영기획 본부 |
| **v1.4 (#19)** | **2026-05-10** | **6개 품질 기준 최종 검토 보강 (엄격 적용)**: ① §3 **각 Story당 두 번째 실패 AC 14건 추가** (56→70 AC, 패스 조건 "Story당 실패 케이스 2개 이상" 충족) ② **F-8.1 OR-Tools를 Sprint 12.1·12.2·12.3 세분화** (Could 항목 1스프린트 구현 가능성 충족) ③ §7 시작에 **ADR 링크** 명시 (개발계획서 §15.1 D1~D21) ④ §7.2 Out of Scope에 **단계별 단서** 추가 (P2 진입 시 In, D8 변경 시 검토). **종합 품질 점수 약 90% → 약 97%** | 경영기획 본부 |
