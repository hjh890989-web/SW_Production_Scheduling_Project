...
/** 성형 자동 스케줄 생성 (T5.4). molding:write + AuditLog. */
export async function generateMoldingScheduleAction(weekStartISO: string): Promise<GenerateResult> {
  ...
  const result = await generateAndSave(weekStartISO);
  ...
  return {
    ok: true,
    message: `자동 생성 완료: ${result.saved}건 배치, 경고 ${result.warnings.length}건`,
    saved: result.saved,
