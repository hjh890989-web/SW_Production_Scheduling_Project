...
import { weekMonday, loadWeekEntries } from '@/lib/scheduler/molding-service';
...
export default async function MoldingPage({ searchParams }: { searchParams: { week?: string } }) {
  ...
  const [equipment, calendar, entries] = await Promise.all([
  ...
  return <MoldingClient weekStart={weekStart} model={model} cellCount={entries.length} />;
}
